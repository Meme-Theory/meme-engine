...
Investigation Arcs are multi-phase — run `/rclab-team` once per phase, gated by prior results:

```bash
# Phase 1: Foundation (fan-out computation)
/rclab-team sessions/session-plan/session-17a-prompt.md --mode compute --team-name s17-phase-a

# Phase 2: Verification gate (after Phase 1 completes)
/rclab-team sessions/session-plan/session-17b-prompt.md --mode compute --team-name s17-phase-b

# Phase 3: Execution (only if Phase 2 gates CLEARED)
/rclab-team sessions/session-plan/session-17c-prompt.md --mode compute --team-name s17-phase-c

# Phase 4: Interpretation (synthesis of all prior phases)
/rclab-team sessions/session-plan/session-17d-prompt.md --mode panel --team-name s17-phase-d

# Post-arc mass review: all agents review the final synthesis
/rclab-review sessions/session-17/session-17-final.md
```
...
/rclab-team session-17a-prompt.md --mode compute    # Foundation
/rclab-team session-17b-prompt.md --mode compute    # Verification gate
/rclab-team session-17c-prompt.md --mode compute    # Gated execution
/rclab-team session-17d-prompt.md --mode panel      # Interpretation

# Review phase (parallel fan-out)
/rclab-review session-17-final.md                   # All 17 agents review
```
