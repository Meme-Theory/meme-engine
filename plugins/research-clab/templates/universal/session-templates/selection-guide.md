...
|:-------|:-------------|:----------|:------|
| A. First Contact | `/rclab-review` | `--agents` subset or full roster | Fan-out, no live team needed |
| B. Adversarial Debate | `/rclab-team` | `--mode panel` | Pre-assign adversarial postures in prompt |
| C. Deep-Dive | `/rclab-team` | `--mode panel` (spec) or `--mode compute` (code) | Panel for specification, compute for execution |
| D. Workshop | `/rclab-team` | `--mode workshop` | One invocation per round, or single prompt with all rounds |
| E. Investigation Arc | `/rclab-team` | `--mode compute` per phase, `--mode panel` for final | Gate check between invocations |
| F. Decisive Computation | `/rclab-team` | `--mode compute` | Wrap-up as separate `--mode panel` invocation |
| G. Mass Parallel Assessment | `/rclab-review` | Full roster, then cross-evals | Master collab via `--mode panel` |
| H. Decision Gate | `/rclab-team` | `--mode compute` | Human reads verdict, picks next branch |
| I. Formalization | `/rclab-team` + `/rclab-review` | `--mode panel` → mass review | Panel catalogs, review validates |

**Rule of thumb**:
- **Need independent reviews?** → `/rclab-review`
- **Need live coordination?** → `/rclab-team`
...
|:-----------|:--------|:--------------|
| Decision Gate → Decisive Computation | Sessions 20a → 20b | `rclab-team --mode compute` → check verdict → `rclab-team --mode compute` |
| Decisive Computation → Mass Parallel Assessment | Sessions 18 → 19d | `rclab-team --mode compute` → `rclab-review` (full roster) |
| Investigation Arc with Workshop sub-session | Session 25 | `rclab-team --mode compute` (phases) → `rclab-team --mode workshop` (one phase) |
| Formalization → Workshop | Sessions 15 → 16 | `rclab-team --mode panel` → `rclab-team --mode workshop` |
| First Contact → Adversarial Debate → Deep-Dive | Sessions 1 → 5 → 6 | `rclab-review` → `rclab-team --mode panel` → `rclab-team --mode panel` |
| Compute → Review → Synthesize | Sessions 28a → 28-review → 28-fusion | `rclab-team --mode compute` → `rclab-review` → `rclab-team --mode panel` |
...
|:-------|:------|:-----|:-----|:---------|:-----------|
| A. First Contact | `/rclab-review` | — | 4-17 | 1-2 sessions | Multi-lens assessment |
| B. Adversarial Debate | `/rclab-team` | `panel` | 3-5 | 1 session | Concessions + settling computations |
| C. Deep-Dive | `/rclab-team` | `panel`/`compute` | 3-5 | 1 session | Computation specification |
| D. Workshop | `/rclab-team` | `workshop` | 6-10 | 1-2 sessions | Ranked action items with specs |
| E. Investigation Arc | `/rclab-team` | `compute`×N | 3-5/phase | Multi-session | Phased deliverables with gates |
| F. Decisive Computation | `/rclab-team` | `compute` | 2-3 | Hours | Gate verdict |
| G. Mass Parallel Assessment | `/rclab-review` | — | 8-17 | 1-2 sessions | Synthesis + master collab |
| H. Decision Gate | `/rclab-team` | `compute` | 2-3 | Hours | Binary routing verdict |
