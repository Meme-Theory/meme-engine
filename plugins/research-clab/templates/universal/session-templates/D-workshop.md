...
Workshops run as sequential rounds — use `/rclab-team --mode workshop`:

```bash
# Launch a workshop from the agenda/prompt document
/rclab-team sessions/session-plan/session-16-workshop-agenda.md --mode workshop

# With explicit team name (recommended for multi-round tracking)
/rclab-team sessions/session-plan/session-16-workshop-agenda.md --mode workshop --team-name workshop-s16

# Dry run to verify round structure and agent pairings
/rclab-team sessions/session-plan/session-16-workshop-agenda.md --mode workshop --dry-run
```
...
/rclab-team sessions/session-plan/session-16-round-1.md --mode workshop --team-name s16-r1

# Round 2: Deep Dives (after Round 1 output exists)
/rclab-team sessions/session-plan/session-16-round-2.md --mode workshop --team-name s16-r2

# Round 3: Action Items (after Round 2 output exists)
/rclab-team sessions/session-plan/session-16-round-3.md --mode workshop --team-name s16-r3

# Final synthesis: switch to rclab-review for mass parallel review of Round 3 output
/rclab-review sessions/session-16/session-16-round-3c-priorities.md --agents workhorse-1,skeptic,calculator,principalist
```

**Combining with `/rclab-review`**: After the workshop's structured rounds, use `/rclab-review` on the final synthesis document to get independent evaluations from agents who didn't participate in the workshop — this catches gaps the workshop team missed.
