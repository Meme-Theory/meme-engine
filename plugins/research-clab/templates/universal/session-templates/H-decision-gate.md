...
Decision gates are minimal compute sprints — use `/rclab-team --mode compute`:

```bash
# Run the gating computation
/rclab-team sessions/session-plan/session-20a-prompt.md --mode compute --team-name s20a-gate

# After the gate verdict, launch the appropriate branch:
# If OPEN:
/rclab-team sessions/session-plan/session-20b-prompt.md --mode compute --team-name s20b-open
# If CLOSED:
/rclab-team sessions/session-plan/session-20b-alt-prompt.md --mode compute --team-name s20b-closed

# Post-gate mass review (optional — have all agents assess the gate outcome)
/rclab-review sessions/session-20/session-20a-decision-gate.md
```
...
/rclab-team sessions/session-plan/session-28-prompt-a.md --mode compute --team-name s28a
...
/rclab-team sessions/session-plan/session-28-prompt-b.md --mode compute --team-name s28b

# Sub-session C: constraint chain completion (reads B's verdicts)
/rclab-team sessions/session-plan/session-28-prompt-c.md --mode compute --team-name s28c

# Post-chain synthesis: mass review of all results
/rclab-review sessions/session-28/session-28c-results.md
```
