...
3. **Source-First, Never Fabricate**: Prefer the `paper-search` MCP when configured (structured metadata, abstracts, PDF download). Then WebFetch the source URL for full text. If both fail, write ONLY the verified metadata you have (title, authors, year, abstract) and mark the rest `[INCOMPLETE -- source not accessible]`. NEVER fill gaps with training knowledge -- a long file invented from memory is hallucinated content disguised as a real reference. A short honest file beats a fabricated one. Don't waste time on 404s or paywalls -- record the failure and move on.
...
### 2. Research Phase
Accept inputs: topic, folder_path, paper_count (default 14), optional paper_list, optional project_context. If a `paper-search` MCP is configured, prefer it for discovery: `search_arxiv` (USE arXiv API field syntax -- `au:Last_First`, `ti:keywords`, `abs:keywords`, `cat:...`, combine with `AND`; a natural-language query returns the latest papers, not the relevant ones), plus `search_google_scholar` / `search_pubmed` / `search_biorxiv` / `search_medrxiv` by domain, and `download_arxiv` / `read_arxiv_paper` for full text. If no `paper-search` MCP is available and no paper_list was provided, fall back to 2-3 parallel WebSearch queries (e.g., `"{topic}" most cited important papers`, `"{topic}" key publications contributions`, `"{topic}" seminal work [domain]`). Compile a chronologically sorted list with title, author(s), year, source URL, and 1-sentence inclusion reason. Selection priority: foundational/seminal papers > most-cited field-shaping papers > project-relevant papers > key experimental papers > modern review articles.
