...
- Author search: `au:LastName_FirstName` (e.g., `au:Hinton_Geoffrey`, `au:Noether_Emmy`)
- Title search: **one `ti:` per word**, joined with AND (e.g., `ti:graph AND ti:neural AND ti:networks`)
- Abstract search: `abs:keyword` (one per word, join with AND)
- All fields: `all:keyword`
- Category: `cat:cs.LG`, `cat:math.CO`, etc.
- Combine fields: `au:Hinton_Geoffrey AND ti:graph AND ti:neural`
- **WRONG**: `ti:graph neural networks` (only `graph` hits title; rest is orphaned, returns garbage)
- **WRONG**: `"Geoffrey Hinton graph networks"` (natural language, returns latest papers, ignores query)
- **RIGHT**: `au:Hinton_Geoffrey AND ti:graph AND ti:neural`
