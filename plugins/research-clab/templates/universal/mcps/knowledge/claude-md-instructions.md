...
**Configuring the canonical-constants bridge.** The shipped server auto-detects a `canonical_constants.py` module at the project's compute directory (`tier0-computation/` or `computation/`) or under `tools/` — the first that exists wins, resolved at server start. If the project has no such module, `get_constant` / `list_constants` / `update_constant` are disabled cleanly; search and entity tools still work. To use a different location, set `CONSTANTS_PATH` in `tools/mcp-servers/knowledge-mcp/server.py`.
