...
**Task**: Install format-specific document templates and configure the `/paper` skill based on the user's output format selection.
...
Question 4 of `/new-research-project` asks the user: "What document format will this project produce?" The answer determines which document templates get installed and how the `/paper` skill behaves.

Currently three format tracks are supported:

| Format | Templates Shipped | Status |
|:-------|:-----------------|:-------|
| **LaTeX** | 14 templates (paper, paper-math, poster, slides, thesis, report, etc.) | Complete |
...
Copy ALL `.tex` files from `${CLAUDE_PLUGIN_ROOT}/templates/universal/document-templates/latex/` into `artifacts/document-templates/latex/`:

```
paper-standard.tex      # Generic article (natbib, amsmath)
paper-math.tex          # amsart + amsthm + cleveref for math manuscripts
...
Copy `${CLAUDE_PLUGIN_ROOT}/templates/universal/document-templates/typst/README.md` into `artifacts/document-templates/typst/`.
...
Copy `${CLAUDE_PLUGIN_ROOT}/templates/universal/document-templates/markdown/README.md` into `artifacts/document-templates/markdown/`.

The `/paper new` command will scaffold markdown documents from general knowledge.
...
```
artifacts/
  document-templates/    # Document templates for /paper new
...
```
| `/paper` | Document toolkit — create, draft, review, cite, compile | Recommended |
```

---

## Step 4: Update `paper` SKILL.md (Format Binding)

The `paper` SKILL.md is already installed by unfold-structure Step 4. It includes format-aware building capabilities. No modification needed — the skill discovers templates at `artifacts/document-templates/` at runtime.
...
- [ ] For LaTeX: 14 `.tex` files + 1 `.bib` file present
- [ ] Root CLAUDE.md references the document-templates directory
- [ ] `/paper` skill is installed (from unfold-structure Step 4)
...
- **Do NOT modify the `paper` SKILL.md** — it ships ready-to-use from the plugin
- **Do NOT install templates for formats the user didn't select** — install only the chosen format
- **Do NOT compile any LaTeX** — that's the user's responsibility via `/paper compile`
