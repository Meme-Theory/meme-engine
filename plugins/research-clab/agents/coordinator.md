...
- `project-name`, `domain`, `research-question`, `output-format`, `hardware-specs`
...
3. **Project-specific files** (Phase 4): Read `{plugin-root}/project-origami/unfold-structure.md` Step 2 and root CLAUDE.md template at `{plugin-root}/templates/claude-md/claude-md-root.md`. Generate:
   - Root `CLAUDE.md` with template variable substitution (project-name, domain, hardware, python)
   - `.claude/settings.local.json` with permissions
   - `.gitignore`
   - `agents.md` registry

4. **Methodology & session framework** (Phase 5): Read `{plugin-root}/project-origami/unfold-methodology.md`. Generate:
...
5. **Team protocol** (Phase 6): Read `{plugin-root}/project-origami/unfold-teams.md`. APPEND team operations protocol to coordinator MEMORY.md (do NOT overwrite — preserve Phase 5 content). Verify clab-team skill exists. Clean stale `~/.claude/teams/` and `~/.claude/tasks/`.

6. **Rules spot-check** (Phase 6 cont.): Read `{plugin-root}/project-origami/unfold-rules.md`. Verify copied rules contain expected content markers.

7. **Infrastructure gate**: Verify all Phase 2-6 outputs exist. Report any missing files.
